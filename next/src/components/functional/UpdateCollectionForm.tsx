...
type UpdateCollectionFormProps = {
  onSubmit: (values: FormData, participations: StudentParticipation[]) => Promise<void>;
...
export default function UpdateCollectionForm({ onSubmit, group: groupFragment, collection: collectionFragment }: UpdateCollectionFormProps) {
  ...
  const initialValues = collection
    ? {
        ...
        learningObjectiveCodes: collection.learningObjectives.map((it) => it.code),
      }
  ...
  return (
    <Formik initialValues={initialValues || defaultInitialValues} onSubmit={handleSubmit}>
      {() => (
        <Card as={Form} display="flex" flexDirection="column" flex="1">
          <Text as="h1" textAlign="center">
            {collection ? "Arvioinnin muokkaus" : "Uusi arviointi"}
          </Text>
          {collection && collection.type.length > 0 && <Text>Vanha tyyppi: {collection?.type}</Text>}
          <FormField
            label="Ympäristö"
            name="environmentCode"
            validate={(value) => (value.length === 0 ? "Ympäristö pitää olla valittu" : "")}
          ...
          <FormField
            ...
            render={({ field: { name }, form: { setFieldValue, setFieldTouched } }) => (
              <LearningObjectiveSelect
                ...
                onChange={(newValue) => {
                  setFieldTouched(name, true);
                  setFieldValue(name, isMultiOption(newValue) && newValue.map((it) => it.code));
                }}
              />
            )}
          />
          <FormField name="date" type="date" label="Päivämäärä" />
          <FormField as={Textarea} label="Muita tietoja" name="description" placeholder="Muita tietoja arviointikertaan liittyen..." />
          <Text as="h2">Oppilaat</Text>
          <StudentParticipationList initialParticipations={participations} onChange={onParticipationsChanged} isDisabled={isSubmitting} mb="5" />
          <Button type="submit" marginTop="auto" isLoading={isSubmitting}>
            {collection ? "Tallenna" : "Siirry arvioimaan"}
          </Button>
          <Text as="a" color="gray.700" mt="3" textTransform="uppercase" textAlign="center" onClick={() => router.back()}>
            Peruuta
          </Text>
        </Card>
      )}
