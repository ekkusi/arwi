...
import { Button, Divider, Flex, Tab, TabList, TabPanel, TabPanels, Tabs, Text, useToast } from "@chakra-ui/react";
...
import LearningObjectivesPieChart, { DataType } from "@/components/functional/LearningObjectivesPieChart";
...
function GroupOverviewPageContent() {
  ...
  const { data } = useSWR<GroupOverviewPage_GetGroupQuery>(`group/${groupId}`, () =>
  ...
  const sortedStudents = group.currentClassYear.students.sort((a, b) => a.name.localeCompare(b.name));

  const sortedCollections = group.currentClassYear.evaluationCollections.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    ...
    <PageWrapper>
      <TopNavigationBar>
        <Popover>
          ...
          <PopoverItem icon={BiTransfer} onClick={() => setIsChangeYearModalOpen(true)}>
            Vaihda vuosiluokkaa
          </PopoverItem>
          <Divider />
          <PopoverItem icon={RiDeleteBin6Line} color="error" onClick={() => setIsDeleteModalOpen(true)}>
            Poista
          </PopoverItem>
        </Popover>
      </TopNavigationBar>
      <ConfirmationModal isOpen={isDeleteModalOpen} awaitBeforeClose onClose={() => setIsDeleteModalOpen(false)} onAccept={() => deleteGroup()}>
      ...
      <CollectionsChart collections={group.currentClassYear.evaluationCollections} />
      <Tabs isFitted>
        <TabList>
          <Tab>Oppilaat</Tab>
          <Tab>Arvioinnit</Tab>
            ...
            {sortedStudents.length > 0 ? (
              <Flex flexDirection="column">
                {sortedStudents.map((student) => (
                  <Card as={Link} key={student.id} py="2" href={`/student/${student.id}`} _notLast={{ mb: 3 }}>
                    {student.name}
                  </Card>
                ))}
              </Flex>
            ...
            {sortedCollections.length > 0 ? (
              <Flex flexDirection="column">
                {sortedCollections.map((collection) => (
                  <Card key={collection.id} as={Link} href={`/collection/${collection.id}`} py="2" _notLast={{ mb: 3 }}>
                    {formatDate(new Date(collection.date))}: {collection.environment.label}
                  </Card>
                ))}
              </Flex>
            ) : (
              <>
                ...
                <Button as={Link} width="100%" href={`/group/${group.id}/create-collection`}>
                  Tee arviointi
                </Button>
              </>
            )}
